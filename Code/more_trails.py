...
def run_trial(exit_blocks, thresholds, trial_name, regenerate_forced=False):
    result_path = f"results_updated/{trial_name}.txt"
    ...
    if retrain_required:
        ...
        for param in model.initial_layers.parameters():
        ...
        os.makedirs("results_updated", exist_ok=True)
...
if __name__ == "__main__":
    print("Running Set2 Exit Block Trials for Exit After Residual Block 2 and 3...")
    for second in range(4, 15):
        run_trial([2, second], [0.6, 1.0], f"set2_exit_blocks_2_{second}", regenerate_forced=True)
        if second > 3:
            run_trial([3, second], [0.6, 1.0], f"set2_exit_blocks_3_{second}", regenerate_forced=True)
    print("\n-----> Selective Set2 Trials Completed!")
